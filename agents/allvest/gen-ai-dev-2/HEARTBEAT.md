...
## 7. Fix Requests from Tester

When you receive a task created by the Tester (titled "Fix: [test/category]" or similar), or when the Tester @-mentions you about failing tests:

1. Read the fix task description — understand what failed, the exact error message, and the test category.
2. Read the tester's comment thread for additional context and reproduction steps.
3. Identify the root cause in your code. **Fix the application code, not the test** — unless the test itself is genuinely wrong (flawed assertion, outdated expectation).
4. Run the failing test locally to verify your fix resolves it: `npm test -- --grep "[test name]"`.
5. Push the fix to your PR branch and update the PR.
6. @-mention the Tester in a comment on the fix task: "@Tester — fix pushed for [failing test / category]. Please re-run the full suite."
7. Mark your fix task as `done`.
8. If you cannot reproduce or fix the issue, set the fix task to `blocked` and comment explaining what you tried, then @-mention the Tester for clarification.

**Do NOT @-mention the CTO for test failures** — the Tester owns the feedback loop. Only escalate to CTO for architectural decisions or organizational blockers.
